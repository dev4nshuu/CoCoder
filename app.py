...
def handle_update(data):
    ...
    userName = data['userName']
    cursor = data['cursor']
    emit('update_text', {'text': text, 'currentTextEditorName': currentTextEditorName,
                         'userName': userName, 'cursor': cursor}, to=room, skip_sid=True)
...
def code_exe(language, code, inputVal):
    ...
    try:
        if language == "py":
            ...
            compile_command = ["g++", "-o", f"{folderPath}/{fileName}_out", filePath]
            compileCode = subprocess.run(compile_command, capture_output=True, text=True)
